	...
	for (int i = 0; i < 26; i++){
		for(int j = 23;j < 83 ; j++){
			...
			if ((LETTER1 + i) == keyboard[x][y]){
				...
				m2[i] = CHECK_ZERO(keyboard[x][y]) * LETTER_SHIFT(keyboard[x][y]-LETTER1) +
...
int main(void)
{
	...
	for (; i < (strlen(line) - 2) ; i++ ){
		...
		char next = (char) tolower((char)line[i+1]);
			printf("cur = %c, next = %c, %d %d %d\n",cur,next,(int)cur - LETTER1,m2[cur - LETTER1],LETTER_SHIFT(next));
		if (cur == next) 
			continue;
		if ((m2[cur - LETTER1] & LETTER_SHIFT(next - LETTER1) ) == 0){
			break_flag = false;
			break;
		}
	}
